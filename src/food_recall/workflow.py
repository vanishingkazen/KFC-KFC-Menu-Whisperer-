...
def create_workflow():
    ...
    # ===== 阶段一：粗排召回 =====
    workflow.add_node("extract_calorie", extract_calorie)
    workflow.add_node("calculate_range", calculate_range)
    workflow.add_node("sql_recall", sql_recall)

    # ===== 阶段二：精排核验 =====
    workflow.add_node("extract_demands", extract_demands)
    workflow.add_node("validate_batch", validate_batch)
